...
public class Win {
    ...
    public void openPage() throws IOException, ClassNotFoundException {
        ...
        setLevel();
        if (nextLevel.equals(Level.LEVEL_1)){
            button.setText("restart");
            buttonActionRestart(button,stage);
        }else {
            buttonActionNext(button, stage);
        }
    ...
    private void buttonActionNext(Button button, Stage stage){
        button.setOnAction(actionEvent -> {
            ...
            UserPage.setLevel(nextLevel);
            try {
                Game game=new Game(nextLevel);
    ...
    private void buttonActionRestart(Button button, Stage stage){
        button.setOnAction(actionEvent -> {
            stage.close();
            Game.setTanks(new ArrayList<>());
            Game.setArmoredTankDestroyed(0);
    ...
    private void setLevel(){
        Level currentLevel = UserPage.getLevel();
        Level[] levels = Level.values();
        nextLevel = levels[(currentLevel.ordinal() + 1) % levels.length];
    }
}
